...
import { Theme, faceInfo, NoticePost, online } from '../components/tool'
...
if(props.theme.isPC) getFace().catch(console.log)
...
async function GetPastPost() {
  ...
  let res = await get_posts(BeginTime.value, BeginTime.value + 7 * 86400)
  // if (res.data.code == 0) {
...
// 更新贡献者提交时间
if(props.theme.isPC) setInterval(() => posters.value.server += 1, 1000)
...
async function NewPost() {
  let data = await get_posts(LastPostTime)
  posters.value = data.online
