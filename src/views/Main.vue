<template>
  <Nav src="https://yun.nana7mi.link/7mi.webp" height="200px" :isCovered="isCovered" @search="searchHandler"
    :theme="theme" :blog="blog">
    <template #login>
      <p :style="{ fontSize: '1.2em', width: '300px' }">{{ blog.name }}</p>
      <!-- color: blog.get_pendant_color() -->
      <el-button type="danger" @click="clean">退出登录</el-button>
    </template>
    <template #logout>
      <img src="https://i0.hdslb.com/bfs/new_dyn/0de10012b4a96d7d4bcd82728f77b2051464240042.png"
  ...
  <div class="content">
    <slot name="left"></slot>
    <div v-infinite-scroll="GetPastPost" :infinite-scroll-immediate="false" :infinite-scroll-distance="100"
      class="post">
      <Post :key="post.platform + post.mid" :blog="post" v-for="post in FilterPosts" />
...
import { get_submitter_info, get_blogs, get_online } from '../components/backend'
import { Blog, Online } from '../components/model'
import { Theme } from '../components/tool'
import { NewBlogger } from '../components/api' // posts, PostModel,
import { Blogger, me } from '../components/api'
...
// 博文数据
const PastPosts = ref<Blog[]>([])
const FuturePosts = ref<Blog[]>([])
const FilterPosts = ref<Blog[]>([])
...
async function GetPastPost() {
  ...
  //   await GetPastPost()
  // }
  PastPosts.value = PastPosts.value.concat(await get_blogs())
  searchHandler("")
}
...
//     get_submitter_info(key).then(result => {
//       let name = result.data.data.name
//       submitters.value.users[name] = data.online.users[key]
//     })
...
function searchHandler(search: string) {
  let TotalPosts = FuturePosts.value.concat(PastPosts.value).filter(
    post => {
      let blog: Blog = post
      let watch = blog.platform + blog.uid
